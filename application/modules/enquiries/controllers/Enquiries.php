...
function create() 
{
    ...
    if ($submit=="Submit") {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data['sent_by'] = 0; //nwere
            $data['opened'] = 0;
            $data['code'] = $this->secure_me->generate_random_string(21);
...
function _fetch_customers_as_options()
{
    ...
    $query = $this->youraccount->get('lastname');
    foreach($query->result() as $row) {
...
SELECT enquiries.*, 
    ...
    youraccount.lastname, 
    youraccount.company
FROM enquiries LEFT JOIN youraccount ON enquiries.sent_by = youraccount.id
...
SELECT enquiries.*, 
    ...
    youraccount.lastname, 
    youraccount.company
FROM enquiries INNER JOIN youraccount ON enquiries.sent_to = youraccount.id
