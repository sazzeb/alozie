...
{
    //upadte/manange group belonging to parent
...
function delete($update_id)
{
    ...
    if ($submit=="Cancel") {
        ...
        $flash_msg = "The ".$entity_name." image was successfully deleted.";
...
function _process_delete($update_id)
{
    ...
    $picture_path = './slider_pics/'.$big_pic;
...
{
    //draw uplaod image, btn etc ontop of the view page 
    $data = $this->fetch_data_from_db($update_id);
    $picture = $data['picture'];
    if($picture == '')
    {
        $data['got_pic'] = FALSE;
        $data['btn_style'] = '';
        $data['btn_info'] = 'No picture has been uploaded so far this slide';
    }else{
        $data['got_pic'] = TRUE;
...
function do_upload($update_id)
{
    ...
    else
    {
        ...
        //redirect the user to a view page;
        redirect('slides/view/'.$update_id);
    }
}
...
{
    // form has been submittec, try to create  new record
    $this->load->module('secure_me');
    $this->secure_me->_allow_only_authorise_persons();
...
{
    //view details regarding and display form
    $this->load->module('secure_me');
    $this->secure_me->_allow_only_authorise_persons();
...
function fetch_data_from_db($update_id) 
{
    ...
    foreach($query->result() as $row) {
        //`target_url`, `picture`, `alt_text`, `item_description`, `big_pic`, `small_pic`, `parent_id`
