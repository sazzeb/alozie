...
function create() 
{
    ...
    $customer_id = $this->secure_me->_get_user_id();
    ...
    if ($submit=="Submit") {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ((!is_numeric($customer_id)) OR ($customer_id==0)) {
                ...
                $customer_id = $this->youraccount->_get_customer_id_from_token($token);
                $not_logged_in = TRUE;
            }
            ...
            $data['opened'] = 0;
            $data['code'] = $this->secure_me->generate_random_string(6);
    ...
    $data['token'] = $this->youraccount->_generate_token($customer_id);
...
function view()
{
    ...
    $code = $this->uri->segment(3);
    $col1 = 'sent_to';
    $value1 = $this->secure_me->_get_user_id();
    $col2 = 'code';
    $value2 = $code;
