<?php
class Igashar_categories extends MX_Controller 
{
...
function view($update_id)
{
    ...
    $data['item_segments'] = $this->setting_component->_get_item_segments();
    $data['query'] = $this->_custom_query($mysql_query);
    $data['update_id'] = $update_id;
    $data['flash'] = $this->session->flashdata('item');
    $data['view_module'] = "igashar_categories";
    $data['view_file'] = "view";
...
SELECT construction_items.item_title, 
    construction_items.item_url,
    construction_items.little_description, 
    construction_items.small_pic
FROM igashar_cat_assign INNER JOIN construction_items ON igashar_cat_assign.item_id = construction_items.id
WHERE igashar_cat_assign.cat_id=$update_id 
...
{
    $mysql_query = "select * from igashar_categories where parent_cat_id=0 order by priority";
    $query = $this->_custom_query($mysql_query);
    foreach($query->result() as $row) {
...
{
    //NOTE: this gets used on igashar_cat_assign
    $mysql_query = "select * from igashar_categories where parent_cat_id!=0 order by parent_cat_id, cat_title";
    $query = $this->_custom_query($mysql_query);
    foreach($query->result() as $row) {
...
{
    $mysql_query = "select * from igashar_categories where parent_cat_id=$parent_cat_id order by priority";
    $data['query'] = $this->_custom_query($mysql_query);
...
function _get_dropdown_options($update_id)
{
    ...
    $mysql_query = "select * from igashar_categories where parent_cat_id=0 and id!=$update_id";
    $query = $this->_custom_query($mysql_query);
    foreach($query->result() as $row) {
...
function create() 
{
    ...
    if ($submit=="Cancel") {
        redirect('igashar_categories/manage');
    }
    ...
    if ($submit=="Submit") {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (is_numeric($update_id)) {
                ...
                redirect('igashar_categories/create/'.$update_id);
            } else {
...
{
    $this->load->model('mdl_igashar_categories');
    $query = $this->mdl_igashar_categories->get($order_by);
    return $query;
}
...
function get_with_limit($limit, $offset, $order_by) 
{
    ...
    $this->load->model('mdl_igashar_categories');
    $query = $this->mdl_igashar_categories->get_with_limit($limit, $offset, $order_by);
    return $query;
}
...
function get_where($id)
{
    ...
    $this->load->model('mdl_igashar_categories');
    $query = $this->mdl_igashar_categories->get_where($id);
    return $query;
}
...
{
    $this->load->model('mdl_igashar_categories');
    $query = $this->mdl_igashar_categories->get_where_custom($col, $value);
    return $query;
}
...
{
    $this->load->model('mdl_igashar_categories');
    $this->mdl_igashar_categories->_insert($data);
}
...
function _update($id, $data)
{
    ...
    $this->load->model('mdl_igashar_categories');
    $this->mdl_igashar_categories->_update($id, $data);
}
...
function _delete($id)
{
    ...
    $this->load->model('mdl_igashar_categories');
    $this->mdl_igashar_categories->_delete($id);
}

function count_where($column, $value) 
{
    $this->load->model('mdl_igashar_categories');
    $count = $this->mdl_igashar_categories->count_where($column, $value);
    return $count;
}
...
{
    $this->load->model('mdl_igashar_categories');
    $max_id = $this->mdl_igashar_categories->get_max();
    return $max_id;
}
...
{
    $this->load->model('mdl_igashar_categories');
    $query = $this->mdl_igashar_categories->_custom_query($mysql_query);
    return $query;
}
