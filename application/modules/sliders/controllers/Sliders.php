...
function _attempt_draw_slider()
{
    ...
    $query = $this->get_where_custom('target_url', $current_url);
    ...
    if($num_rows>0)
    {
        ...
        $data['query'] = $this->slides->get_where_custom('parent_id',$parent_id);
...
function delete($update_id)
{
    ...
    if ($submit=="Cancel") {
        ...
        $flash_msg = "The sliderwas successfully deleted.";
...
{
    //draw the slider blocks that are on the homepage
    ...
    if ($submit=="Submit") {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (is_numeric($update_id)) {
                ...
                $flash_msg = "The slider was successfully added.";
                $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('sliders/create/'.$update_id);
            }
