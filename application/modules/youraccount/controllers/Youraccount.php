...
class Youraccount extends MX_Controller
{
    ...
    function _process_create_account()
    {
        ...
        if($this->_insert($data))
        {
            ...
            $value = '<div class="alert alert-success" style="text-align: center" role="alert">'.$flash_msg.'</div>';
            $this->session->set_flashdata('item', $value);
            redirect('youraccount/login');
        }else{
            $flash_msg = "Your registration failed, you can contact admiistrator at '".$this->config->item('iga_solution')."' to assist you further";
            $value = '<div class="alert alert-warning" style="text-align: center" role="alert">'.$flash_msg.'</div>';
            $this->session->set_flashdata('item', $value);
            $this->start();
    }
    ...
    function _set_session($email_address)
    {

        $mysql_query = "select username,firstname,lastname,email,company where email = '{$email_address}'";
        $query = $this->_custom_query($mysql_query);
        $num_rows = $query->rows();
        $sess_data = array(
            'username' => $num_rows->username,
            'firstname' => $num_rows->firstname,
            'lastname' => $num_rows->lastname,
            'email' => $num_rows->email,
            'company' => $num_rows->company,
        );
        $this->session->set_userdata($sess_data);
    }

    function _get_email_code()
    {
        $data = $this->fetch_data_from_post();
        $email = $data['email'];
        ...
        $generate_code = $username.$new_email.$firstname;
        $email_code = md5($generate_code);
        return $email_code;
    }
    ...
    function _send_validation_email()
    {
        ...
        $names = $fisrtname. ' ' . $lastname;
        $email_code = $this->_get_email_code();
    ...
    {
        $mysql_query = "SELECT email, date_made, firstname FROM youraccount WHERE email = '{$email_address}' LIMIT 1";
        $result = $this->_custom_query($mysql_query);
        $row = $result->row();
        if($result->num_rows() == 1 && $row->firstname)
        {
            if(md5((string)$row->date_made) == $email_code)
            {
